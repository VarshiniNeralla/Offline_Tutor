        ...
        try {
            ...
            if (data.mode === 'quiz') {
                ...
                try {
                    ...
                    parsedQuestions = typeof responseStr === 'string' ? JSON.parse(responseStr) : responseStr;
                } catch (e) {
        ...
        >
            ...
            <div className="quiz-options-grid">
                {[5, 10, 15].map(num => (
                    ...
                    >
                        ...
                        <span className="label">{t.quiz.questionsLabel}</span>
                    </button>
                ))}
            </div>
            ...
            <div className="quiz-actions">
                <button
                    ...
                    onClick={onStart}
                >
                    {t.quiz.start} <ArrowRight size={20} />
                </button>
            </div>
        </motion.div>
    );
        ...
        >
            <div className="quiz-game-header">
                <div className="progress-pill">
                    {t.quiz.questionLabel} {currentQIndex + 1} / {questions.length}
                </div>
            ...
            <div className="quiz-progress-bar">
                <div
                    ...
                    style={{ width: `${questions.length > 0 ? ((currentQIndex + 1) / questions.length) * 100 : 0}%` }}
                ></div>
            </div>
            ...
            <div className="quiz-footer">
                <button
                    ...
                    onClick={handleNext}
                >
                    {currentQIndex === questions.length - 1 ? t.oral.finishTest : t.oral.nextQuestion}
                </button>
            </div>
        </motion.div>
    );
...
const QuizResults = ({ questions, userAnswers, timeElapsed, onRetry, onExit }) => {
    ...
    let badge = { text: t.quiz.keepTrying, color: "orange" };
    if (percentage >= 90) badge = { text: t.quiz.excellent, color: "green" };
    else if (percentage >= 70) badge = { text: t.quiz.goodJob, color: "blue" };
    ...
    return (
        ...
        >
            ...
            <div className="stats-grid">
                ...
                <div className="stat-card">
                    <span className="label">{t.quiz.time}</span>
                    <span className="value">{formatTime(timeElapsed)}</span>
                </div>
            </div>

            <div className="review-section">
                <h3>{t.quiz.reviewAnswers}</h3>
                        ...
                        <div key={idx} className="review-item">
                            <div className="review-header">
                                ...
                                {userAnswers[idx] === q.correct_index ?
                                    <span className="status correct">{t.quiz.correct}</span> :
                                    <span className="status wrong">{t.quiz.incorrect}</span>
                                }
                            </div>
                            <p className="review-q">{q.question}</p>
                            <p className="review-ans">
                                {t.quiz.correctLabel}: <strong>{q.options[q.correct_index]}</strong>
                            </p>
                        </div>
                    ))}
                </div>
            ...
            <div className="results-actions">
                <button className="secondary-btn" onClick={onExit}>{t.dashboard.back}</button>
                <button className="primary-btn" onClick={onRetry}><RotateCcw size={18} /> {t.quiz.retry}</button>
            </div>
        </motion.div>
    );
};
